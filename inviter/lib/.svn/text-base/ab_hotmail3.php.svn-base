...
Hotmail & MSN contacts importer (Using Windows Live Contacts API 1.0 and delegated auth)

This class currently support only  delegated authentication on a domain that hasn't
been registered with Microsoft's Azure Service Developer Portal.
(If the domain has been registered, the returned consent token is encrypted, and requires
PHP5 or better to be implemented)
...
//    var $cumulusHost = "https://cumulus.services.live.com";
		...
		foreach ($parts as $part) {
			...
			if (count($va)==2) {
				...
				if ($key=='delt') {
					...
					return abi_set_error(_ABI_FAILED,'Encrypted delegation token not supported in this version. Please remove your domain from the Windows Azure service');
				}
	...
	function retrieveContacts () {
		...
		if ($this->lastStatusCode != 200) {
			return abi_set_error(_ABI_AUTHENTICATION_FAILED,'Unauthorized : '.$this->lastStatusCode);
		}
	...
	function fetchContacts ($loginemail, $password) {
		return abi_set_error(_ABI_FAILED,'Please call authenticate() with the ConsentToken from Windows Live Delegated Authentication to login');
	}
}
